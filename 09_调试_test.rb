...
3 使用 byebug gem 调试【检查源码】
{
	...
	var 		显示变量及其值
		...
		local		当前范围内的局部变量
	next 		执行一行或几行,不进入方法
	step 		每步执行一个（或多个）块或方法 ，逐方法调试
	continue 	Ctrl+D 直接跳到下一个byebug,如果后面没有则退出byebug 
	list(l) 	跳行命令
		l 45		跳到第45行
		l=			跳到当前停止位置
	display 	跟踪某个变量 display @user
	finish  	退出
...
4 使用 web-console gem 调试 【页面控制台】
{
	https://ruby-china.github.io/rails-guides/debugging_rails_applications.html
	Web Console 的作用与 byebug 有点类似，不过它在浏览器中运行。
	在任何页面中都可以在视图或控制器的上下文中请求控制台。控制台在 HTML 内容下面渲染。
	1、激活方法：
	在控制器中 
	...
	<h2>New Post</h2>
	2、 审查变量
	可以调用 instance_variables 列出当前上下文中的全部实例变量。
	如果想列出全部局部变量，调用 local_variables。
